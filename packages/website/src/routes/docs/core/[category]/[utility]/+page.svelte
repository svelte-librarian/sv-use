...
<main id="content" class="relative flex w-full flex-col">
	...
	{#if data.Component}
		...
		<div
			id="demo-wrapper"
			class="demo-wrapper relative overflow-auto rounded-lg bg-zinc-100 p-5 dark:bg-zinc-800"
		>
			<DemoComponent />
		</div>
	{/if}
	...
	{#key $page.url.pathname}
		...
		>
			<summary
				id="type-definitions"
				class="scroll-mt-12 py-5 text-2xl font-semibold lg:scroll-mt-16"
			>
				Type definitions
			</summary>
...
<style lang="postcss">
	...
	:global(#content pre code span[data-highlighted-line]) {
		background-color: rgba(200, 200, 255, 0.1);
	}

	:global(#content pre code > [data-line]) {
		padding: 2px 20px;
	}

	:global(#content pre code[data-line-numbers] > [data-line]::before) {
	...
	:global(#content pre),
	:global(#content pre code),
	:global(#content pre code span) {
		color: var(--shiki-light);
		background-color: var(--shiki-light-bg);
	}

	:global(html.dark #content pre),
	:global(html.dark #content pre code),
	:global(html.dark #content pre code span) {
	...
	:global(html.dark #content *:not(pre) code) {
		@apply dark:bg-svelte-dark;
	}
</style>
